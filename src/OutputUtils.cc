                                          ...
                                          hid_t file_id)
{
  ...
  std::vector<float> local_data(count);
  std::vector<float> global_data(count_total);
  ...
  // get all data
  MPI_Gatherv(local_data.data(), count, MPI_FLOAT, global_data.data(),
              counts.data(), offsets.data(), MPI_FLOAT, 0, comm);

  // write to disk
  if(myProc == 0)
    WriteField(file_id, global_data.data(), label, components, ntracers);
}
